...
<svelte:head>
	<title>{$tChirho('common.readChirho.titleChirho')} - {$tChirho('common.appNameChirho')}</title>
</svelte:head>

<main class="min-h-screen bg-slate-50 p-8">
	<div class="mx-auto max-w-4xl">
		<h1 class="text-3xl font-bold text-slate-900">{$tChirho('common.readChirho.selectLanguageChirho')}</h1>
		<p class="mt-2 text-slate-600">{$tChirho('common.readChirho.descriptionChirho')}</p>
		...
		<div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each dataChirho.languagesChirho as languageChirho}
				...
				>
					...
					{#if languageChirho.verseCountChirho > 0}
						<div class="mt-2 flex gap-3 text-xs text-slate-600">
							<span class="flex items-center gap-1">
								...
								{formatCountChirho(languageChirho.verseCountChirho)} verses
							</span>
							<span class="flex items-center gap-1">
								<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
								</svg>
								{formatCountChirho(languageChirho.wordCountChirho)} words
							</span>
						</div>
					{:else}
						<p class="mt-2 text-xs text-slate-400 italic">{$tChirho('common.readChirho.noTranslationsChirho')}</p>
					{/if}
				</a>
			{/each}
		</div>
		...
		{#if dataChirho.languagesChirho.length === 0}
			<p class="mt-8 text-slate-500">{$tChirho('common.messagesChirho.noDataChirho')}</p>
		{/if}
	</div>
</main>
