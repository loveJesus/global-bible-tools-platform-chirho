...
const LANGUAGE_NAMES_CHIRHO: Record<string, { nameChirho: string; nativeNameChirho: string }> = {
  ...
  nep: { nameChirho: "Nepali", nativeNameChirho: "नेपाली" },
  ukr: { nameChirho: "Ukrainian", nativeNameChirho: "Українська" },
  guj: { nameChirho: "Gujarati", nativeNameChirho: "ગુજરાતી" },
  srp: { nameChirho: "Serbian", nativeNameChirho: "Srpski" },
...
function generateConfFileChirho(
  ...
  translationTypeChirho?: string
): string {
  const dateChirho = new Date().toISOString().split("T")[0];
  const editionLabelChirho = translationTypeChirho === "readers" ? " (Natural Reading)" : "";
  const editionDescChirho = translationTypeChirho === "readers"
    ? "Natural reading interlinear Bible translation from Greek/Hebrew."
    : "Word-by-word interlinear Bible translation from Greek/Hebrew.";
...
# Module Information
Description=Love Jesus Machine Translation Interlinear - ${langInfoChirho.nameChirho}${editionLabelChirho}
About=${editionDescChirho}\\par\\par\\
 ...
 */
function getTranslationDirsChirho(langCodeChirho: string, translationTypeChirho?: string): string[] {
  // If type specified, use only that type; otherwise prefer readers then terse
  const typesToTryChirho = translationTypeChirho === "readers"
    ? ["readers-chirho"]
    : translationTypeChirho === "terse"
    ? ["terse-chirho"]
    : ["readers-chirho", "terse-chirho"];

  for (const typeChirho of typesToTryChirho) {
    const baseDirChirho = join(TRANSLATIONS_DIR_CHIRHO, typeChirho, `${langCodeChirho}-chirho`);
    if (existsSync(baseDirChirho)) {
      const dirsChirho = readdirSync(baseDirChirho)
        .filter(dChirho => dChirho.endsWith("-chirho"))
        .map(dChirho => join(baseDirChirho, dChirho));
      if (dirsChirho.length > 0) {
        console.log(`Using ${typeChirho} translations (${dirsChirho.length} books)`);
        return dirsChirho;
      }
    }
  }

  // Fallback: old structure translations-chirho/<book>-<lang>-chirho/
 ...
 * New structure: "genesis-chirho" → "genesis"
 * Old structure: "matthew-spa-chirho" → "matthew"
 */
function extractBookNameChirho(dirNameChirho: string, langCodeChirho: string): string {
  const baseChirho = basename(dirNameChirho);
  // New structure: <book>-chirho
  const suffixOldChirho = `-${langCodeChirho}-chirho`;
  let bookChirho = baseChirho.endsWith(suffixOldChirho)
    ? baseChirho.replace(suffixOldChirho, "")
    : baseChirho.replace(/-chirho$/, "");
...
async function processTranslationsChirho(
  ...
  const dirsChirho = getTranslationDirsChirho(langCodeChirho, translationTypeChirho);
  ...
  for (const dirChirho of dirsChirho) {
    ...
    // Get all SQL files in this directory (per-verse c*-chirho.sql or all-verses-chirho.sql)
    const sqlFilesChirho = readdirSync(dirChirho)
      .filter(fChirho => fChirho.endsWith("-chirho.sql") && (fChirho.startsWith("c") || fChirho === "all-verses-chirho.sql"))
...
async function mainChirho(): Promise<void> {
  ...
  if (argsChirho.length === 0) {
    ...
    console.log("");
    console.log("Usage:");
    console.log("  bun run tools-chirho/generate-sword-module-chirho.ts <language> [--books book1,book2] [--type terse|readers]");
    console.log("");
    ...
    console.log("  bun run tools-chirho/generate-sword-module-chirho.ts ben --type readers");
    console.log("  bun run tools-chirho/generate-sword-module-chirho.ts ben --type terse");
    console.log("");
  ...
  // Parse --type flag (terse or readers)
  let translationTypeChirho: string | undefined;
  const typeIdxChirho = argsChirho.indexOf("--type");
  if (typeIdxChirho !== -1 && argsChirho[typeIdxChirho + 1]) {
    translationTypeChirho = argsChirho[typeIdxChirho + 1].toLowerCase();
    if (translationTypeChirho !== "terse" && translationTypeChirho !== "readers") {
      console.error(`Invalid --type: ${translationTypeChirho}. Must be 'terse' or 'readers'.`);
      process.exit(1);
    }
    console.log(`Using ${translationTypeChirho} translation type`);
  }
  ...
  // Module ID (e.g., LJMTIntSpaChirho for terse, LJMTIntSpaRChirho for readers)
  const readersTagChirho = translationTypeChirho === "readers" ? "R" : "";
  const moduleIdChirho = `LJMTInt${langCodeChirho.charAt(0).toUpperCase()}${langCodeChirho.slice(1)}${readersTagChirho}Chirho`;
  ...
  try {
    ...
    const booksChirho = await processTranslationsChirho(sqlChirho, langCodeChirho, filterBooksChirho, translationTypeChirho);
